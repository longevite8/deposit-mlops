...
task = Task.init(
    ...
    task_name=TEMPLATE_DEPLOY_SERVING_NAME,
...
params = task.connect(
    {
        ...
        "endpoint_version": CLEARML_SERVING_ENDPOINT_VERSION,
        "preprocess": CLEARML_SERVING_PREPROCESS,
        "engine": CLEARML_SERVING_ENGINE,
        "alias": CLEARML_SERVING_ALIAS,
...
if not params["promote_task_id"]:
    task.get_logger().report_text("Template creation mode.")
    task.close()
    raise SystemExit(0)

promote_task = Task.get_task(task_id=params["promote_task_id"])
promote_summary = wait_for_artifact(
    promote_task,
    "promote_summary",
    max_retries=10,
    wait_interval=2.0,
...
promote_lineage = wait_for_artifact(
    promote_task,
    "promote_lineage",
    max_retries=10,
    wait_interval=2.0,
...
deploy_lineage = {
    ...
    "feature_dataset_id": promote_lineage.get("feature_dataset_id", ""),
...
task.upload_artifact("deploy_serving_summary", deploy_summary)
task.upload_artifact("deploy_serving_lineage", deploy_lineage)
task.get_logger().report_text(
    f"Deployed model {model_id} to {deployment['endpoint_url']}"
)
task.flush()
task.close()
