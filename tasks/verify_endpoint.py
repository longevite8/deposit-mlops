...
task = Task.init(
    ...
    task_name=TEMPLATE_VERIFY_ENDPOINT_NAME,
    task_type=Task.TaskTypes.qc,
)

params = task.connect(
    {
        "deploy_serving_task_id": "",
        "base_url": CLEARML_SERVING_BASE_URL,
        "endpoint_prefix": CLEARML_SERVING_ENDPOINT_PREFIX,
        "endpoint": CLEARML_SERVING_ENDPOINT,
        "version": CLEARML_SERVING_ENDPOINT_VERSION,
        "payload_json": "",
        "horizon": FORECAST_HORIZON,
    }
)

if not params["deploy_serving_task_id"]:
    task.get_logger().report_text("Template creation mode.")
    task.close()
    raise SystemExit(0)

deploy_task = Task.get_task(task_id=params["deploy_serving_task_id"])
deploy_summary = wait_for_artifact(
    deploy_task,
    "deploy_serving_summary",
    max_retries=10,
    wait_interval=2.0,
    logger_obj=task,
)
...
if not deploy_summary.get("deployed", False):
    verify_summary = {
        "ok": False,
        "skipped": True,
        "reason": "Serving deployment was skipped or failed upstream.",
    }
else:
    verify_summary = run_check(
        argparse.Namespace(
            base_url=str(params["base_url"]),
            endpoint_prefix=str(params["endpoint_prefix"]),
...
verify_lineage = {
    "verify_endpoint_task_id": task.id,
    "deploy_serving_task_id": deploy_task.id,
    "champion_model_id": deploy_lineage.get("champion_model_id", ""),
...
task.upload_artifact("verify_endpoint_summary", verify_summary)
task.upload_artifact("verify_endpoint_lineage", verify_lineage)
task.get_logger().report_single_value("endpoint_ok", int(bool(verify_summary.get("ok"))))
task.get_logger().report_text(f"Endpoint verification summary: {verify_summary}")
task.flush()
task.close()

if not verify_summary.get("ok", False) and not verify_summary.get("skipped", False):
    raise SystemExit(1)
